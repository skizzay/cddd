...
class Cddd(ConanFile):
    ...
    default_options = "static=True", "gtest:shared=False"

    def config(self):
        if (self.settings.build_type == "Debug"):
            self.options.enable_testing = True

        if (hasattr(self.options, "enable_testing") and self.options["enable_testing"]):
            self.requires("gtest/1.7.0@lasote/stable")

    def build(self):
        cmake = CMake(self.settings)
        link_type = "-DBUILD_SHARED_LIBS=ON" if not self.options.static else ""
        lto_option = "-DGCC_LINK_TIME_OPTIMIZATION=ON" if self._lto_enabled else ""
        enable_testing = "-DBUILD_TESTS=ON" if self._testing_enabled else ""
        ...
        if not exists(join(build_dir, "CMakeCache.txt")):
            mkdir(build_dir)
            self.output.info("cd %s && cmake %s %s %s" % (build_dir, self._directory_of_self, cmake.command_line, extra_flags))
            self.run("cd %s && cmake %s %s %s" % (build_dir, self._directory_of_self, cmake.command_line, extra_flags))
        self.output.info("cmake --build %s %s" % (build_dir, cmake.build_config))
    ...
    def _testing_enabled(self):
        return hasattr(self.options, "enable_testing") and self.options["enable_testing"]
