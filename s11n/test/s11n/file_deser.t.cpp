...
TEST_CASE("writing to the file advances the position", "[file_sink,file_deser]") {
    auto target = file_sink::temporary();
...
TEST_CASE("default constructed file sink is not open", "[file_sink,file_deser]") {
    file_sink const target;
    REQUIRE_FALSE(target.is_open());
}

TEST_CASE("File can write (sink) and read (source)", "[file,file_deser]") {
