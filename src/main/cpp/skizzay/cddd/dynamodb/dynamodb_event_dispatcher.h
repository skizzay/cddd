...
namespace event_dispatcher_details_ {} // namespace event_dispatcher_details_

template <concepts::domain_event... DomainEvents> struct event_dispatcher {
  ...
  void dispatch(item_type const &item,
                event_visitor<DomainEvents...> &visitor) {
    try {
  ...
  template <std::invocable<item_type const &> Translator>
  requires(std::same_as<std::invoke_result_t<Translator, item_type const &>,
                        DomainEvents> ||
           ...) void register_translator(std::string event_type_name,
    ...
    if (handlers_.contains(event_type_name)) {
      ...
      auto handler = [translator = std::move(translator)](
        ...
        static_cast<event_visitor_interface<
            std::invoke_result_t<Translator, item_type const &>> &>(v)
