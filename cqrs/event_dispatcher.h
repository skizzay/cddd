...
template<class Alloc=std::allocator<std::pair<const std::type_index, domain_event_handler>>>
class basic_domain_event_dispatcher {
   typedef std::hash<std::type_index> index_hash;
   typedef std::equal_to<std::type_index> index_equals;
   typedef std::unordered_map<std::type_index, domain_event_handler, index_hash, index_equals, Alloc> dispatch_table;
...
public:
   ...
   basic_domain_event_dispatcher(const basic_domain_event_dispatcher &) = delete;
   basic_domain_event_dispatcher(basic_domain_event_dispatcher &&) = default;
   explicit inline basic_domain_event_dispatcher(const allocator_type &alloc=allocator_type{}) :
      handlers(alloc)
   {
   }

   basic_domain_event_dispatcher &operator =(const basic_domain_event_dispatcher &) = delete;
   basic_domain_event_dispatcher &operator =(basic_domain_event_dispatcher &&) = default;
   ...
   inline void add_handler(std::type_index type, domain_event_handler f) {
...
typedef basic_domain_event_dispatcher<> domain_event_dispatcher;
